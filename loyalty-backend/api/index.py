...
def pull_dataset():
    """Pull processed loyalty dataset từ HuggingFace."""
    global loyalty_dataset_df
    try:
        from datasets import load_dataset
        print(f"Đang pull loyalty dataset: {DATASET_REF}...")
        ds = load_dataset(DATASET_REF, token=HF_TOKEN, split="train")
        loyalty_dataset_df = ds.to_pandas()
        print(f"✅ Loyalty dataset: {loyalty_dataset_df.shape}")
    except Exception as e:
        print(f"Warning: Loyalty dataset lỗi ({e})")
        local_path = "models/loyalty-dataset"
        if os.path.exists(local_path):
            from datasets import load_from_disk
            loyalty_dataset_df = load_from_disk(local_path).to_pandas()
            print(f"✅ Loyalty dataset (local): {loyalty_dataset_df.shape}")

def pull_raw_dataset():
    """Pull raw Online Retail II dataset từ HuggingFace, trích xuất danh sách customers."""
    global raw_df, customers_df
    try:
        from datasets import load_dataset
        print(f"Đang pull raw dataset: {RAW_DATASET_REF}...")
        ds = load_dataset(RAW_DATASET_REF, token=HF_TOKEN, split="train")
        raw_df = ds.to_pandas()
        print(f"✅ Raw dataset: {raw_df.shape}")
        _build_customers()
    except Exception as e:
        print(f"Warning: Raw dataset chưa có trên HF ({e}). Thử đọc local CSV...")
